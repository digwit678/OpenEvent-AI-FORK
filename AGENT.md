# OpenEvent-AI Agent Guide (Codex)
...
3.  **During Coding:**
    *   **Use Skills:** Leverage `.codex/skills/` (especially `oe-architectural-guardrails`).
...
4.  **Before Committing:**
    ...
    *   **E2E Check:** Verify critical flows (Billing -> Deposit -> HIL) via Playwright or manual UI test.
    *   **Deploy Safety:** If pushing to `main`, strictly follow `docs/plans/active/UPDATE_MAIN.md` (Verify Critical Subset).
    *   **Update Docs:** Add entry to `DEV_CHANGELOG.md` and update `TEAM_GUIDE.md` if a bug was fixed.
    *   **Cleanup:** Run code cleanup/formatter.
