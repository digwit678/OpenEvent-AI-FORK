...
def now_iso() -> str:
    return datetime.now(tz=TZ).astimezone(ZoneInfo("UTC")).isoformat(timespec="seconds").replace("+00:00", "Z")
...
def load_rooms_config(path: Path | None = None) -> List[Dict[str, Any]]:
    candidate = path or Path(__file__).with_name("rooms.json")
...
def collect_conflicts(
    ...
    buffer_before = timedelta(minutes=int(room.get("buffer_before_min", 0)))
    buffer_after = timedelta(minutes=int(room.get("buffer_after_min", 0)))
...
def build_options_for_reply(rooms_checked: Sequence[Dict[str, Any]]) -> List[Dict[str, str]]:
    ...
    if options:
        return options
    for room in rooms_checked:
        if room.get("availability") != "partial":
            continue
...
def run_availability_workflow(
    ...
    try:
        ...
        # Missing or invalid time window → draft clarification reply instead of hard-failing
        reason = str(exc)
        event_data["Status"] = "Needs Clarification"

        # Minimal draft per bubble tone
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the availability workflow for a confirmed event.")
    parser.add_argument("event_id", help="Event identifier from events_database.json")
    parser.add_argument(
        "--rooms",
        type=Path,
        default=Path(__file__).with_name("rooms.json"),
        help="Path to rooms configuration JSON.",
    )
    args = parser.parse_args()

    ensure_calendar_dir()
    calendar = CalendarAdapter()
    client_gui = ClientGUIAdapter()
    run_availability_workflow(args.event_id, calendar, client_gui, args.rooms)
