...
function QnAPageContent() {
  ...
  useEffect(() => {
    // If snapshot_id is present, fetch from snapshot API
    if (snapshotId) {
      fetch(`http://localhost:8000/api/snapshots/${snapshotId}`)
        .then(res => res.json())
        .then(snapshot => {
          if (snapshot.error) {
          ...
          // Transform snapshot data to QnAData format
          ...
          // If snapshot has full_lines, we can display those too
          if (snapshotData.full_lines) {
            // Could parse these into items if needed
          }
    ...
    function fetchByParams() {
      ...
      // Try new universal endpoint first, fallback to old endpoint
      const newUrl = `/api/qna?${params.toString()}`
      const oldUrl = `/api/test-data/qna?${params.toString()}`

      fetch(newUrl)
        .then(res => res.json())
        .then(newData => {
          // If new endpoint returns success, use it
          if (newData.success && newData.handled) {
            // Transform new format to old format for display
        ...
        .catch(err => {
          ...
          // Try old endpoint as final fallback
          fetch(oldUrl)
            .then(res => res.json())
            .then(oldData => {
              setData(oldData)
  ...
  const handleCategoryChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (cat === 'all') {
       router.push('/info/qna')
    } else {
       router.push(`/info/qna?category=${encodeURIComponent(cat)}`)
    }
  }
