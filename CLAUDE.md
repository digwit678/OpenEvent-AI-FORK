...
1.  **Start of Session:**
    *   If it exists, read `docs/daily_scrum/session_primer.md` first (cheap delta summary generated by automation).
        *   In Claude Code, you can run `/primer` (see `.claude/commands/primer.md`).
    *   Only if needed, read `DEV_CHANGELOG.md` & `docs/guides/TEAM_GUIDE.md` (Check "High-Risk Areas").
...
*   **Sync to Main:** Use `oe-update-main` skill (MANDATORY - re-reads `docs/plans/active/UPDATE_MAIN.md`)

## 6. Main Branch Sync Protocol

> **CRITICAL:** When user says "commit to main", "sync to main", or "update main", you **MUST** use the `oe-update-main` skill.

This skill:
1. Forces re-reading of `docs/plans/active/UPDATE_MAIN.md`
2. Prevents accidental frontend sync
3. Ensures all verification steps run before push

**Never sync from memory.** The UPDATE_MAIN.md file is the source of truth.
